/*
 /// <summary>
 /// app.modules.mft_v2.controllers - mftAgentReportingCtrl.js
 /// Base MFT V2 Agent Reporting Controller
 /// Abstract Controller that is the parent of all MFT Agent V2 Reporting Controllers
 ///
...
define(['modules/mft_v2/module', 'moment', 'lodash','appCustomConfig'], function (module, moment, lodash, appCustomConfig) {
	...
	module.registerController('mftAgentReportingCtrl', ['$scope', '$log', '$state', 'uiSvc', 'transactionReportingSvc', 'mftv2DataSvc', 'userSvc',function($scope, $log, $state, uiSvc, transactionReportingSvc, mftv2DataSvc, userSvc)
	{
		// initialize the tabs
		var _this = this;
		_this.functions = {};
        ...
        _this.functions.navigate = function (id, grid)
        {
            ...
            // routine to invoke the agent drill
            $state.go("app.mft_v2.agent", {id: id});
        };
        ...
        _this.functions.refreshData = function()
        {
            ...
            mftv2DataSvc.agentSearch(filterObject).then(function(result)
            {
                // update the counts
                 _this.model.counts = mftv2DataSvc.parseAgentCounts(result.total);
                _this.model.data = mftv2DataSvc.parseAgentGridData(result.records);
                _this.model.filter.applied = true;
            }).catch(function(err)
            {
                $log.error("Unable to Refresh MFT Agent Display", err);
            }).finally(function()
            {
        ...
        _this.functions.initialize = function()
        {
            ...
            _this.model.filter.queueManagers = [];
            _this.model.filter.os = [];
            _this.model.filter.hosts = [];
            _this.model.filterName = "mftv2Agent";
