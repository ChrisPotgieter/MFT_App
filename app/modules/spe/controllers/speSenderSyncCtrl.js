/*
 /// <summary>
 /// app.modules.spe.controllers - speSenderSyncCtrl
 /// SPE Controller for Managing Sender Profile Syncronization with the ITXA Server
 ///
...
define(['modules/spe/module', 'lodash'], function (module, lodash) {
    ...
    module.registerController('speSenderSyncCtrl', ['$uibModalInstance', '$scope', '$log', '$interval', 'speDataSvc','adminDataSvc','userSvc', 'uiSvc', 'cacheDataSvc', function ($uibModalInstance, $scope, $log, $interval, speDataSvc, adminDataSvc, userSvc, uiSvc, cacheDataSvc)
    {
       var _this = this;

       _this.titleData = {icon:"fa fa-exchange", desc: "Sender Profile Syncronization"};
       _this.modalResult = null;
       _this.dataModel = {user_id: userSvc.getProfile().id, companyId: userSvc.getOrgInfo().companyId, sender_id: $scope.senderId};
       _this.progressInfo = {icon:"fa fa-cog", showProgress: true, title: "ITXA Sender Profile Synronization ", description:"Preparing for Syncronization", perc: "1%", allowOK:false, allowCancel: false, showLineDisplay: false, showOK: true, showCancel: false, initialized: false, OKText: "OK"};
       _this.timer = null;

        _this.prepare = function()
        {
            // routine to prepare the sync
            speDataSvc.senderSync(_this.dataModel).then(function(result)
            {
            ...
            {
                $log.error("Unable to Prepare for Sender Profile Syncronization", result);
            })
        };
        ...
        _this.updateProgress = function(model)
        {
            ...
            if (status == uiSvc.backgroundProcessStates.COMPLETED)
            {
               ...
               _this.progressInfo.title = "ITXA Sender Profile Syncronization";
               _this.progressInfo.perc = "100%";
               _this.progressInfo.allowOK = true;
               _this.progressInfo.allowCancel = true;
               _this.stopTimer();

               // update the cache to reflect any changes to the hipaa validation list
                cacheDataSvc.initializeLists(true);
                return;
            }
            ...
            if (status == uiSvc.backgroundProcessStates.INERROR)
            {
                ...
                _this.progressInfo.description = model.description;
                _this.progressInfo.perc = "100%";
                _this.progressInfo.lines.push("Syncronization Failed");
                _this.stopTimer();
                return;
            }
        };
